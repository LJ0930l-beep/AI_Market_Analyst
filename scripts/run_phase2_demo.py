...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--settle", action="store_true", help="settle one deterministic future bar into Outcome rows")
    ...
    for symbol in args.symbols:
        try:
            result = service.analyze(instrument_for(symbol), timeframe=args.timeframe, limit=120)
    ...
    print(json.dumps({"phase": 2, "mode": args.mode, "llm": args.llm, "results": output, "failures": failures, "counts": store.counts()}, indent=2))
