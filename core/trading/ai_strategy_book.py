...
TEMPLATES = [
    {"id": "aggressive_impulse", "name": "闪电动量 · 5m 激进", "style": "AGGRESSIVE", "scan_interval_minutes": 5, "order_preference": "AUTO",
     ...
     "profile": _profile(strategy_id="aggressive_impulse", family="MOMENTUM_LIQUIDITY_SWEEP", candidate_strategy_ids=["liquidity_sweep", "ema_trend"], signal_timeframe="5m", context_timeframes=["15m", "1h"], required_confirmations=2, minimum_signal_score=3, volume_ratio_min=1.18, atr_stop_multiple=1.80, major_stop_floor_pct=1.50, alt_stop_floor_pct=2.50, minimum_net_rr=1.6, target_r_multiples=[1.8, 3.0], order_preference="AUTO", limit_priority=True, limit_ttl_seconds=480, max_limit_distance_pct=0.85, allow_market_entry=True, market_min_trigger_completion=70, allow_future_limit=True, max_entries_per_hour=3, cooldown_minutes=5, news_mode="RISK_FILTER_WITH_SYMBOL_CATALYST"),
     "sections": {**DEFAULT_SECTIONS, "role": "你是专注于加密货币合约的多空双向实战交易员。以已收盘 5m 为触发周期、15m 与 1h 为结构背景，结合全市场资金面与新闻情绪，捕捉 EMA20 动量突破与流动性扫荡反转机会。执行防插针结构化宽止损与动态仓位自适应风控，拒绝死板百分比与过度等待。", "frequency": "由系统每 5 分钟对齐触发评估，信号周期为已收盘 5m，背景周期为 15m 与 1h。每轮先管理已有持仓，再从候选池中挑选唯一最优的单笔机会；宁可一笔做透，不要分散下单。",
      ...
      "2. 【全 NO_TRIGGER 绝不等于必须 WAIT】：独立依据 5m 触发与 15m/1h 背景：(a) EMA20 动量突破——5m 放量站上/跌破 EMA20 或 resistance20 后优先在回测位挂单；(b) 流动性扫荡——5m 刺破 support20/resistance20 后收回结构内可反向开仓；(c) 箱体边缘——靠近 15m 支撑阻力时预埋限价单。\n"
      ...
      "custom_prompt": "系统自动预埋限价或市价进场。市价与限价开仓门槛均为 70%。完成度达到 70% 即可开单：当价格处于入场区内支持市价（MARKET）快速开仓；当需要回测或避免追高时，自动在突破回测位或 EMA20 回踩位挂出限价单（LIMIT），严禁在无持仓且完成度达 70% 时无故放弃开单选择 WAIT。不要无脑追求极端胜率，70% 完成度即可执行。美股开市正常交易，优先以关键位预埋限价单入场。"}},
    {"id": "aggressive_breakout", "name": "趋势回测 · 15m 激进", "style": "AGGRESSIVE", "scan_interval_minutes": 15, "order_preference": "AUTO",
     ...
     "sections": {**DEFAULT_SECTIONS, "frequency": "由系统每 15 分钟评估。结合 1h 与 15m 已收盘 K 线，先检查已有持仓保护，自适应识别趋势跟踪或箱体高抛低吸机会。",
      ...
      "decision_process": "评估 1h 宏观与 15m 局部；优先在突破回测位预埋限价，满足市价条件才可即时成交；输出结构化 JSON。",
